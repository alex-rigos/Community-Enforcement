...
const stratVector = map(x->x.strategyName,strategies)
...
function makePopulation(pop::Vector{Vector{Any}})
    reduce(vcat,reduce(vcat,map(y->map(x->agent(stratByName(y[1],strategies)),1:y[2]),pop)))
end
...
function punishment!(prodList::Vector{Int},enf::Int,list::Vector{<:Agent})
    ...
    for prod in prodList
        ...
        if enforcerPunishesThisProducer == list[prod].cooperated
            ...
            # println("we got here $(enforcer.strategy)")
            # println("The $(enforcer.strategy) now has karmaI = $(enforcer.karmaI)")
        end
    end
...
function interactEnf!(enf1::Int, enf2::Int,list::Vector{<:Agent})
    ...
    # checkType([enf1,enf2],Enforcer,list)
    ...
    # mis1 = mistake(probAttackMistake)
    # want1 = list[enf1].attacksif(list[enf2])
    # attacks1 = want1 == !mis1
    if verbose
        println("I am a $(list[enf1].strategy) facing someone with karmaI = $(list[enf2].karmaI)")
        println("Did I make a mistake 1? $(mis1). Did I want to attack? $(want1). Did I attack? $(attacks1)")
    end
    checkRulesStep3!(enf1,attacks1,enf2,list)
    
    # mis2 = mistake(probAttackMistake)
    # want2 = list[enf2].attacksif(list[enf1])
    # attacks2 = want2 == !mis2
    if verbose
        println("I am a $(list[enf2].strategy) facing someone with karmaI = $(list[enf1].karmaI)")
        println("Did I make a mistake 2? $(mis2). Did I want to attack? $(want2). Did I attack? $(attacks2)")
    end
    ...
    elseif attacks2 && !attacks1
     ...
     # Check status for enf1 who took action attacks1 against enf2 (and the other way round)
    
    
    
end

#Check for transgressions of enf1 who meets enf2 and takes action attacks1. No detection errors here?
...
function redemption!(list::Vector{<:Agent})
    ...
    if !isempty(enfInd)
        for enf in enfInd
            # enf = list[i]
            # if enf.strategy=="peacekeeper"
            #     println(enf.karmaI)
            # end
            if list[enf].karmaI>0 && list[enf].okWithRuleI
                list[enf].karmaI -= 1
                # if enf.karmaI==0
                #     println("YAY! $(enf.strategy)")
                # end
            end
...
#     prods = getAgentIndices(list,Producer)
#     for prod in prods
#         mistakes!(prod,list)
#     end
# end
# function mistakes!(prod::Int,list::Vector{<:Agent})
#     checkType(prod,Producer,list)
#     list[prod].cooperated = list[prod].cooperate == !mistake(probProduceMistake) 
# end
...
function selection!(list::Vector{<:Agent},k::Int,revVec::Vector{Vector{Any}})
    typeSet = map(x->x[1],revVec)  # Vector of agent types
    typeWeights = Weights(map(x->x[2],revVec))  # Vector probs that each agent type revises
    typesToRevise = sample(typeSet, typeWeights, k)  # Randomly draw k agent types
    fvec = fitnessVector(list)  # Vector of ["strategyname", fitness] pairs for all strategies
    while !isempty(typesToRevise)
        ...
        stratSet = strategies[findall(x->x.agentType <:type,strategies)]  # Vector of strategies of the drawn type
